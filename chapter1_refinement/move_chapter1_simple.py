...
def transfer_section(src_start, src_end, dest_idx, desc):
    ...
    for i in range(src_start, src_end + 1):
        if i < len(doc.paragraphs):
            ...
            # کپی کامل element
            copied_paras.append(deepcopy(para._element))
    ...
    parent = dest_para._element.getparent()
    
    for elem in reversed(copied_paras):
        parent.insert_before(elem, dest_para._element)
    
    print(f"   ✓ Transferred {len(copied_paras)} paragraphs")
    return True
